...
function listenIpcMain(mainwindow){
    ...
    ipcMain.on('zoom-window',(ev, str)=>{
        if(['in','close'].includes(str)){
            mainwindow.mainWindow.minimize()
        }
        if(str=='out'){
            mainwindow.mainWindow.maxmize()
        }
        if(str=='top'){
            const flag = mainwindow.mainWindow.isAlwaysOntop()
            mainwindow.mainWindow.setAlwaysOpTop(!flag)
        }
    })
}
module.exports = {
